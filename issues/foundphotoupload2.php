...
//  $ID=mysqli_real_escape_string($db, );
  ...
  if(isset($_POST['upload'])) {
  	$image_found2 = $_FILES['image_found2']['name'];
    $fnameFinal= "";
  ...
  echo $fnameFinal;
  	...
  	$target ="faceRecogV4Found/labeled_images/".$fnameFinal. '/'.basename($image_found2);

  	$sql = "UPDATE found SET
      image_found2= '$image_found2' WHERE foundId= '$ID'";
    mysqli_query($db, $sql);
    /*if($sql)
    {
      echo "successfull";
    ...
    else
    {
  	...
  	if (move_uploaded_file($_FILES['image_found2']['tmp_name'], $target)) {
...
<div id="content">
  ...
  echo "<form action='foundphotoupload2.php?ID=$ID' method='POST' enctype='multipart/form-data'>" ?>
  	<input type="hidden" name="size" value="1000000">
  	<div>
  	  <input type="file" name="image_found2">
  	</div>
